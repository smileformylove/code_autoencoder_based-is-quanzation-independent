...
class Codec(object):
	...
	def encoder(self, z_hat, sigma_z, y_hat, mu_y, sigma_y, mu_z=0):
		image = self.pre_pic()
		fileobj = open(self.output, 'ab+')
		...
		for h_idx in range(y_hat.shape[2]):
		    ...
		    for w_idx in range(y_hat.shape[3]):
		        for ch_idx in range(y_hat.shape[1]):
		            mu_val = mu_y[0, ch_idx, 0, 0] + 0
		            sigma_val = sigma_y[0, ch_idx, 0, 0]
	...
	def decoder(self, sigma_z, mu_y, sigma_y, mu_z=0):
	    ...
	    return y_hat.transpose([0, 2, 3, 1])
